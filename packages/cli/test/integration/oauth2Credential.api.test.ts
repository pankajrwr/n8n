...
beforeAll(async () => {
	app = await utils.initTestServer({
		endpointGroups: ['credentials', 'oauth2-credential'],
		applyAuth: true,
	});
...
test('GET /auth', async () => {
	...
	// const { id, name, type, nodesAccess, data: encryptedData } = credResponse.body.data;

	console.log('id: ', id);

	const oauthResponse = await authOwnerAgent.get(`/oauth2-credential/auth?credentialId=${id}`);

	console.log(oauthResponse.statusCode);
	console.log(oauthResponse.body);
	console.log(oauthResponse.error);
	console.log(Object.keys(oauthResponse));
});
